...
expected_form_states: List[Dict[str, Set[Any]]] = [
    ...
    {"level": {"850", "1000", "500"}, "param": {"T"}, "step": {"48", "36"}, "number": {"1", "2", "3"}},  # 4
    {"level": {"850", "1000", "500"}, "param": {"T"}, "step": {"48", "36"}, "number": {"1", "2", "3"}},  # 5
    {"level": {"1000", "500"}, "param": {"Z"}, "step": {"24", "48"}, "number": {"1", "2", "3"}},  # 6
    {"level": {"500", "850", "1000"}, "param": {"T", "Z"}, "step": {"24", "48", "36"}, "number": {"1", "2", "3"}},  # 7
    {"level": {"850", "500"}, "param": {"T"}, "step": {"48", "36"}, "number": {"1", "2", "3"}},  # 8
    {"level": {"850", "500"}, "param": {"T"}, "step": {"48", "36"}, "number": {"1", "2", "3"}},  # 9
    {"level": {"1000", "850", "500"}, "param": {"T", "Z"}, "step": {"48", "36", "24"}, "number": {"1", "2", "3"}}  # 10
]
...
def test_parse_valid_combinations() -> None:
    assert parsed_valid_combinations == \
           constrictor.parse_valid_combinations(valid_combinations)
    assert [{}] == constrictor.parse_valid_combinations([{}])


def test_parse_possible_selections() -> None:
    assert parsed_possible_selections == \
           constrictor.parse_possible_selections(possible_selections)
...
def test_parse_current_selection() -> None:
    for i in range(len(current_selections)):
        try:
            assert parsed_current_selections[i] == \
                constrictor.parse_current_selection(current_selections[i])
        except AssertionError:
            print(f"Test {i} failed!")
            raise AssertionError
