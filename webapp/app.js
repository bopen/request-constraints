$(document).ready(() => {
  ...
  const getSelection = () => {
    ...
    $form.find('input:checked').each((i, el) => {
      selected[el.name] = selected[el.name] ? [...selected[el.name], el.value]: [el.value];
  ...
  const updateValidityState = (state) => {
  ...
  const prepareForm = () => {
    ...
    Object.entries(config).forEach(([name, values]) => {
      ...
      values.forEach((value) => {
        const newField = $(`<label class="field"><input type="checkbox" name="${name}" value="${value}"> ${value}</label>`);
